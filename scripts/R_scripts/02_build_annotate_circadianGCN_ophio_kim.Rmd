...
```{r define_genes_of_interest, eval=F}
...
rhy.24 <-
  tbl(ejtk.db, paste0(sample.name,"_LD_zscores_24h")) %>% 
...
```{r find_genes_in_network_1, eval=F}
...
names(list2) <- paste0(sample.name, c("-24h"))
sapply(list2, length)
...
```{r plot_gom_1v2, echo = FALSE, fig.align='center', fig.cap='Gene-clusters with rhythmic genes', out.width="65%", eval=F}
